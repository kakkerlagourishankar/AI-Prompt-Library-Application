  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <nav className="border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-lg fixed w-full z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-x-3">
            ...
            <div>
              <div className="font-bold text-2xl tracking-tighter">promptvault</div>
              <div className="text-[10px] text-zinc-500 -mt-1">AI IMAGE LIBRARY</div>
            </div>
          </div>

          <div className="flex items-center gap-x-8 text-sm">
            <div className="flex items-center gap-x-8">
              <a href="#library" className="hover:text-violet-400 transition-colors">Library</a>
              <a href="#stats" className="hover:text-violet-400 transition-colors">Stats</a>
              <button 
      ...
      <div className="pt-20 max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col items-center text-center pt-12 pb-16 border-b border-zinc-800">
          <div className="inline-flex items-center gap-x-2 px-4 py-1.5 bg-zinc-900 border border-zinc-700 rounded-3xl text-xs mb-6">
            ...
            SIMULATED FULL-STACK • POSTGRES + REDIS + DOCKER
          </div>
          ...
          <p className="max-w-md text-zinc-400 text-lg">
            Store, organize, and reuse your best image generation prompts.<br />
            View counts powered by simulated Redis.
          </p>
          
          <div className="flex gap-8 mt-10">
            <div className="text-center">
              <div className="text-4xl font-mono font-semibold text-violet-400">{prompts.length}</div>
              <div className="text-xs text-zinc-500 tracking-widest mt-1">PROMPTS</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-mono font-semibold text-fuchsia-400">{totalViews}</div>
              <div className="text-xs text-zinc-500 tracking-widest mt-1">TOTAL VIEWS</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-mono font-semibold text-cyan-400">3</div>
              <div className="text-xs text-zinc-500 tracking-widest mt-1">API ENDPOINTS</div>
            </div>
          </div>
        </div>
      ...
      {isDetailOpen && selectedPrompt && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4" onClick={closeDetail}>
          <div 
          ...
          >
            <div className="px-8 pt-8 pb-6 border-b border-zinc-700 flex items-start justify-between">
              <div>
                <div className="flex items-center gap-x-3">
                  <div className={cn(
                    ...
                    {getComplexityLabel(selectedPrompt.complexity)} • {selectedPrompt.complexity}
                  </div>
                  <div className="flex items-center gap-1.5 text-emerald-400 text-sm font-medium">
                    <Star className="w-4 h-4 fill-current" /> {selectedPrompt.views} VIEWS
                    <span className="text-[10px] text-zinc-500">(REDIS COUNTER)</span>
                  </div>
                </div>
              ...
              <button onClick={closeDetail} className="text-zinc-400 hover:text-white">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-8">
              ...
              <div className="mt-8 grid grid-cols-2 gap-4 text-sm">
                <div>
                  <div className="text-zinc-500 text-xs mb-1 tracking-widest">CREATED</div>
                  <div className="font-medium">{formatDate(selectedPrompt.createdAt)}</div>
                </div>
                <div>
                  <div className="text-zinc-500 text-xs mb-1 tracking-widest">VIEW COUNT</div>
                  <div className="font-mono text-4xl font-semibold text-white tabular-nums">
                    {selectedPrompt.views}
                  </div>
                  <div className="text-xs text-emerald-400">+1 on every visit</div>
                </div>
              </div>
            </div>
            ...
            <div className="px-8 py-4 text-[10px] text-center text-zinc-500 border-t border-zinc-800 bg-zinc-950">
              This detail view triggers a simulated Redis INCR command on the view counter. Data persists in browser localStorage simulating PostgreSQL.
            </div>
          </div>
        </div>
      )}
      ...
      {/* Add Prompt Modal */}
      {isAddOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-6" onClick={() => setIsAddOpen(false)}>
          <div 
          ...
          >
            <div className="px-8 py-7 border-b border-zinc-700 flex items-center justify-between">
              ...
              <button onClick={() => setIsAddOpen(false)} className="text-zinc-400">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-8 space-y-8">
              <div>
                <label className="block text-xs tracking-widest text-zinc-400 mb-2">TITLE</label>
                <input
                  type="text"
                  ...
                  className={cn(
                    ...
                    formErrors.title ? "border-red-500" : "border-zinc-700 focus:border-violet-500"
                  )}
                  placeholder="Midjourney style title"
                />
                {formErrors.title && <p className="text-red-400 text-xs mt-2">{formErrors.title}</p>}
              </div>
              
              <div>
                <label className="block text-xs tracking-widest text-zinc-400 mb-2">FULL PROMPT CONTENT</label>
                <textarea
                  ...
                  className={cn(
                    ...
                    formErrors.content ? "border-red-500" : "border-zinc-700 focus:border-violet-500"
                  )}
              ...
              <div>
                <label className="block text-xs tracking-widest text-zinc-400 mb-3">COMPLEXITY (1-10)</label>
                <div className="flex items-center gap-4">
                  <input 
                    ...
                    onChange={(e) => {
                      setNewPrompt({...newPrompt, complexity: parseInt(e.target.value)});
                      if (formErrors.complexity) setFormErrors({...formErrors, complexity: ''});
                    }}
                ...
                {formErrors.complexity && <p className="text-red-400 text-xs mt-2">{formErrors.complexity}</p>}
      ...
      <footer className="bg-black border-t border-zinc-900 py-12 text-center text-xs text-zinc-500">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-y-6">
            <div>
              Built as a React + Vite + Tailwind demo that simulates the full Django + Angular + PostgreSQL + Redis + Docker stack described.
            </div>
            
            <div className="flex items-center gap-x-8">
              <div className="flex items-center gap-x-5">
                <div className="px-3 py-1 bg-zinc-900 rounded-xl">POSTGRES</div>
                <div className="px-3 py-1 bg-zinc-900 rounded-xl">REDIS</div>
                <div className="px-3 py-1 bg-zinc-900 rounded-xl">DJANGO</div>
              </div>
              <div>→</div>
              <div className="font-mono text-emerald-300">DEMO MODE</div>
            </div>
          </div>
          
          <div className="mt-10 text-[10px] opacity-50">
            View counts increment on every detail view. Data is saved in browser. Refresh to reset to initial state if desired.
          </div>
        </div>
      </footer>
    </div>
